...
function ticketForWbText(event) {
  ...
  prompt(
    ...
    `Здравствуйте.
Проверили данные вашего магазина. В API-ключе у некоторых продаж нет данных о сумме, которую оплатил клиент и нет суммы, которую переведет WB за данную продажу.

Для решения расхождения в API-ключе рекомендуем <a href="https://seller.wildberries.ru/service-desk-v2/requests/history">обратиться в поддержку wildberries</a>  (Категорию выберите <b>"API"</b>, тему: <b>"Ошибки в получаемой информации по API"</b>) и попросить их скорректировать данные в API-ключе вашего магазина.
...
${itogInfoApi()} продаж с подобной проблемой ${countNull}шт.</em> 
<em>Просьба скорректировать данные и добавить информацию в API-ключ.
Ответ о том, что данные в аналитических отчетах (в том числе и API) носят промежуточный характер и могут содержать неточную информацию, т.к. являются оперативными и динамически изменяются прошу не писать, поскольку в данных, которые передаются имеется ошибки, в частности не у всех товаров в API-ключе указана сумма покупки и сумма к переводу от WB.</em> 
  
<em> Строка вызова: ${getAPI("sales")}</em>`
  );
}
